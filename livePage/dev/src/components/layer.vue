<template>
  <div>
    ...
    <transition name='fade'>
      <div class='layerShare' v-show='layerShare' @click='clearLayer'></div>
    </transition>
    ...
    <transition name='layerAni'>
      <div class='layerDrawIn' v-show='layerDrawIn'>
        ...
        <div class="btn" @click="boundBtn"></div>
      </div>
    </transition>
...
export default {
  ...
  methods: {
    ...
    callLayerIn(bounsInfo) {
      ...
      if (bounsInfo) {
        this.bounsName = bounsInfo.name
        this.bounsImg = {
          'background': 'url('+bounsInfo.img+') left top no-repeat',
...
.layerShare {
  background: url('./../assets/img/intell/shareText.png') left top no-repeat;
  .rback(25rem,
  30.27rem);
  position: fixed;
  top: 0;
